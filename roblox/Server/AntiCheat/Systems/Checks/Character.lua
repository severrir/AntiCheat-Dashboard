...
local Character = { Name = "ACCharacter", Feature = "Character", Rate = "hot" }
...
function Character.Step(profile, now)
	...
	if hum.Parent ~= char then
		if char:FindFirstChildOfClass("Humanoid") or hum.Health > 0 then
			Trust.Flag(profile, "Character", 35, { kind = "HumanoidSwap" })
			respawn(profile)
		end
		return
	end
	...
	if hum.Health > hum.MaxHealth + 0.01 then
		Trust.Flag(profile, "Character", 30, { kind = "Godmode", hp = hum.Health, max = hum.MaxHealth })
		hum.Health = hum.MaxHealth
		return
	end

	if root.Parent ~= char then
		Trust.Flag(profile, "Character", 25, { kind = "RootRemoved" })
		respawn(profile)
		return
	end
	...
	if root.Size ~= profile.rootSize then
		Trust.Flag(profile, "Character", 25, { kind = "RootResized" })
	...
	if now - profile.lastLimbCheck > 2 then
		...
		if parts < profile.partCount then
			Trust.Flag(profile, "Character", 15, { kind = "LimbRemoved", parts = parts, expected = profile.partCount })
...
function Character:Start()
	...
	Framework.Get("ACScheduler").Add(Character)
end

...
local RingBuffer = require(AC.Lib.RingBuffer)
...
local Timing = { Name = "ACTiming", Feature = "Timing", Rate = "cold" }
...
function Timing.Record(profile, key, now)
	...
	for key, entry in profile.timing do
		...
		if n >= MIN_SAMPLES then
			...
			if mean > 0 and 1 / mean >= MIN_RATE and cv < T.TimingMinCV then
				Trust.Flag(profile, "Timing", 12, { kind = "Macro", remote = key, cv = cv, rate = 1 / mean })
			end
			gaps:Clear()
		end
	end
...
function Timing:Start()
	...
	Framework.Get("ACScheduler").Add(Timing)
end
